    ...
    if ($(slider).attr("data-id") == "movie") {
      ...
      $(slider)
        ...
        .each((_, item) => {
          ...
          const [__, season_count, latest_episode] = $(item)
            .find(".metadata span")
            .map(
              (_, el) =>
                +$(el)
  ...
  $("main section.slider").each((_, sli) => {
    ...
    $(sli)
      ...
      .each((_, item) => {
        ...
        const [type, string2, string3] = $(item)
          .find(".metadata span")
          .map((_, el) => $(el).text())
          .get();
        ...
        if (type == "Movie") {
          ...
          sliderItems.push({
            ...
            type: "tv",
            title,
            season_count: +string2.replace(/[^0-9]/g, ""),
            latest_episode: +string2.replace(/[^0-9]/g, ""),
            quality,
          });
        }
      });
