...
export class TidalAPI {
  ...
  private async fetch<T>(
    ...
    try {
      ...
      return {
        error:
          error.response?.data?.userMessage || error.message || "Failed to fetch from Tidal API",
  ...
  public async search(
    query: string,
    limit: number = 20,
    types: string = "TRACKS,ALBUMS,ARTISTS,PLAYLISTS,VIDEOS",
  ): Promise<any> {
  ...
  public async getTrackStreaming(
    id: string,
  ...
  public async getVideoStreaming(
    id: string,
    ...
    if (sessionId) {
      const [preview, audio, metadata] = await Promise.all([
        previewPromise,
        this.fetch(
          `/videos/${id}/playbackinfopostpaywall`,
          {
  ...
  public async getTrackPlaybackInfoPostPaywall(
    id: string,
    audioQuality: string = "HI_RES",
    sessionId?: string,
  ): Promise<any> {
  ...
  public async getVideoPlaybackInfoPostPaywall(
    id: string,
    videoQuality: string = "HIGH",
    sessionId?: string,
  ): Promise<any> {
